...
class DigiflazzService
{
    ...
    public function processTransaction(Transaction $trx): array
    {
        ...
        // Pre-check saldo kas Digiflazz dari cache jika ada
        $cachedBalance = Cache::get('digiflazz_current_balance');
        $costPrice = (float) ($trx->productItem->original_price ?? 0);
        if ($cachedBalance !== null && $cachedBalance < $costPrice && $costPrice > 0) {
            Log::critical("EMERGENCY: Saldo Digiflazz tidak mencukupi (Tersedia: Rp {$cachedBalance} < Dibutuhkan: Rp {$costPrice}) untuk memproses invoice {$refId}. Transaksi dialihkan ke antrean manual.");
            $trx->update([
                'delivery_status' => 'pending',
                'provider_response' => [
                    'status' => 'insufficient_balance',
                    'message' => 'Saldo kas provider habis/kritis. Transaksi ditangguhkan.',
                ],
            ]);

            return ['status' => 'insufficient_balance', 'message' => 'Saldo provider habis'];
        }
        ...
        try {
            ...
            // Evaluasi jika response Digiflazz menyatakan saldo tidak cukup (RC 41 / 42 atau pesan saldo)
            $rc = $result['data']['rc'] ?? '';
            $msg = strtolower($result['data']['message'] ?? '');
            if (in_array($rc, ['41', '42']) || str_contains($msg, 'saldo') || str_contains($msg, 'cut off')) {
                Log::critical("EMERGENCY: Provider Digiflazz menolak transaksi {$refId} karena saldo admin habis (RC: {$rc}, Pesan: {$msg}). Transaksi ditangguhkan.");
                $trx->update([
                    'delivery_status' => 'pending',
                    'provider_response' => $result['data'] ?? $result,
                ]);

                return ['status' => 'insufficient_balance', 'message' => 'Saldo admin Digiflazz habis'];
            }

            if (isset($result['data']['deposit'])) {
                $currentBalance = (int) $result['data']['deposit'];
                Cache::put('digiflazz_current_balance', $currentBalance, now()->addHours(6));
                $this->evaluateBalanceThreshold($currentBalance);
            }

            return $this->handleProviderResponse($trx, $result);
        } catch (\Throwable $e) {
            Log::warning("Digiflazz Connection / Timeout Failover: {$refId} - {$e->getMessage()}");

            // Failover aman: dialihkan ke status antrean asinkron (processing/pending)
            $trx->update([
                'delivery_status' => 'processing',
                'provider_response' => [
                    'failover' => true,
                    'error' => 'API Timeout / Provider Maintenance (>5s). Transaksi dialihkan ke antrean asinkron.',
            ...
            return [
                'status' => 'pending',
                'failover' => true,
                'message' => 'Provider timeout / maintenance. Transaksi ditahan dalam antrean aman.',
            ];
        }
    }
