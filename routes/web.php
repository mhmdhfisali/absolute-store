...
Route::middleware([
    ...
    // Member Dashboard: Riwayat Pesanan, Saldo, Akun Favorit & Deposit Tracker
    Route::get('/dashboard', function () {
        if (Auth::user()->isAdmin()) {
            return redirect()->route('admin.dashboard');
        }

        $myTransactions = Transaction::where('contact_email_or_phone', Auth::user()->email)
            ->with(['productItem.product', 'paymentMethod'])
            ->latest()
            ->paginate(10);

        $savedAccounts = Auth::user()->savedAccounts()->with('product')->get();
        $products = Product::where('is_active', true)->orderBy('name')->get();
        $paymentMethods = PaymentMethod::where('is_active', true)->get();
        $myDeposits = Auth::user()->deposits()->with('paymentMethod')->latest()->take(10)->get();

        return view('user-dashboard', compact('myTransactions', 'savedAccounts', 'products', 'paymentMethods', 'myDeposits'));
    })->name('dashboard');
...
Route::middleware([
    ...
    Route::post('/transactions/{invoice}/retry', function ($invoice, DigiflazzService $digiflazzService, WhatsAppService $waService) {
        ...
        if (env('DIGIFLAZZ_USERNAME')) {
            ...
            if ($status === 'sukses') {
                return back()->with('success', "Order {$trx->invoice_number} berhasil diproses ulang! SN: " . ($trx->fresh()->serial_number ?? '-'));
            } elseif ($status === 'pending') {
                return back()->with('success', "Order {$trx->invoice_number} sedang diproses oleh provider.");
            } else {
